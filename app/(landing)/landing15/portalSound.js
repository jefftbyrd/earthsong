...
export const portalSound = (p5) => {
  ...
  p5.setup = () => {
    ...
    let downloadButton = p5.createButton('Download shapes data');
    downloadButton.mousePressed(downloadShapes);
  }; // END SETUP
  ...
  function downloadShapes() {
    ...
    // Save the JSON file.
    // p5.saveJSON(data, 'state.json');
    p5.save(sounds2, 'sounds2.json');
  }
  ...
  class Shape {
    ...
    rollover(px, py) {
      ...
      if (p5.keyIsPressed === true) {
        if (p5.key === 'w' && d < 100) {
          rate += 0.02;
        } else if (p5.key === 's' && d < 100) {
          rate -= 0.02;
        } else if (p5.key === 'd' && d < 100) {
          rate = 1;
        }
      }
      if (rate > 4) {
      ...
      multiPlayer.player(this.id).playbackRate = rate;
      // if (d < this.diameter) {
      //   // boolean flag
